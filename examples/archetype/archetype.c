...
int main() {
	...
	printf("Player[0]: pos=(%.1f,%.1f,%.1f) vel=(%.2f,%.2f,%.2f) health=%.1f drag=%.2f\n", players->pos[0].x,
	       players->pos[0].y, players->pos[0].z, players->vel[0].x, players->vel[0].y, players->vel[0].z,
	       players->health[0], players->drag);
	printf("Enemy[0]: pos=(%.1f,%.1f,%.1f) vel=(%.2f,%.2f,%.2f) aggression=%.2f\n", enemies->pos[0].x,
	       enemies->pos[0].y, enemies->pos[0].z, enemies->vel[0].x, enemies->vel[0].y, enemies->vel[0].z,
	       enemies->aggression);
	...
	printf("Player[0]: pos=(%.2f,%.2f,%.2f) vel=%.6f\n", players->pos[0].x, players->pos[0].y, players->pos[0].z,
	       sqrt(players->vel[0].x * players->vel[0].x + players->vel[0].y * players->vel[0].y +
	            players->vel[0].z * players->vel[0].z));
	printf("Enemy[0]: pos=(%.2f,%.2f,%.2f) vel=%.6f\n", enemies->pos[0].x, enemies->pos[0].y, enemies->pos[0].z,
	       sqrt(enemies->vel[0].x * enemies->vel[0].x + enemies->vel[0].y * enemies->vel[0].y +
