...
static TokenKind keyword_kind(const char *start, size_t length) {
	...
	/* `map` is the per-element data-parallel kernel keyword (renamed from `sys`, now retired). */
	...
	if (length == 3 && strncmp(start, "own", 3) == 0) {
		return TOK_OWN;
	}
...
Token lexer_next_token(Lexer *lexer) {
	...
	case '#': {
		...
		if (wlen == 6 && strncmp(word, "import", 6) == 0)
			...
			k = TOK_HASH_LINK;
		else if (wlen == 8 && strncmp(word, "schedule", 8) == 0)
			k = TOK_HASH_SCHEDULE;
