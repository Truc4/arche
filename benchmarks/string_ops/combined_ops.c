...
char *generate_string(int id) {
	...
	snprintf(str, MAX_STRING_LEN, "entity_%d_name", id);
	return str;
}
...
void method1_remove(Method1 *m, int index) {
	if (index < 0 || index >= m->count || m->count == 0)
		return;
...
Method2 method2_create(int initial_count) {
	...
	for (int i = 0; i < initial_count; i++) {
		...
		snprintf(temp, MAX_STRING_LEN, "entity_%d_name", i);
		int len = strlen(temp);
		m.lengths[i] = len;
		m.offsets[i] = pos;
...
void method2_add(Method2 *m, int entity_id) {
	...
	char temp[MAX_STRING_LEN];
	snprintf(temp, MAX_STRING_LEN, "entity_%d_name", entity_id);
	int len = strlen(temp);
...
int main() {
	printf("String Benchmark (COMBINED OPS) - %d iterations\n", ITERATIONS);
	printf("Operation: uppercase + add 10%% + delete 8%% per iteration\n\n");
	...
	for (int iter = 0; iter < ITERATIONS; iter++) {
		...
		// Add/remove only (no uppercase for now)
		int to_delete = m1.count / 12;       // ~8%
		int to_add = (INITIAL_STRINGS / 10); // ~10%
	...
	// Method 2
	printf("Method 2: Contiguous (computed offsets) Contiguous buffer + lengths\n");
	seed = 42;
	Timer t2 = timer_start();
	for (int iter = 0; iter < ITERATIONS; iter++) {
		Method2 m2 = method2_create(INITIAL_STRINGS);

		int to_delete = m2.count / 12;
		int to_add = (INITIAL_STRINGS / 10);
	...
	// Output JSON
	FILE *out = fopen("results/combined_ops_results.json", "w");
	fprintf(out, "{\n");
	fprintf(out, "  \"benchmark\": \"combined_ops\",\n");
	fprintf(out, "  \"initial_strings\": %d,\n", INITIAL_STRINGS);
	fprintf(out, "  \"iterations\": %d,\n", ITERATIONS);
	fprintf(out, "  \"operation\": \"uppercase + add 10%% + delete 8%% per iteration\",\n");
	fprintf(out, "  \"methods\": [\n");
	...
	printf("Results saved to results/combined_ops_results.json\n");
	return 0;
}
