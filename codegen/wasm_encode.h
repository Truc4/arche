...
typedef struct { uint8_t *data; size_t len, cap; } WasmBuf;
...
// ---- module builder ----
typedef struct {
  WasmBuf types;   int type_count;
  WasmBuf imports; int import_count;   // all imports are funcs, in order → func indices 0..import_count-1
  WasmBuf funcs;   int func_count;     // defined funcs' type indices; indices start at import_count
  WasmBuf globals; int global_count;
  WasmBuf exports; int export_count;
  WasmBuf code;    int code_count;     // defined func bodies (parallel to funcs)
  WasmBuf data;    int data_count;
