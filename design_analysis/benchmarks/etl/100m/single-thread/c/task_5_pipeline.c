/* Task 5 (C baseline): multi-step pipeline.
 * Filter quantity > 0 AND price > 10.0, then accumulate price * quantity.
 */
...
int main(int argc, char **argv) {
	...
	while (p < end) {
		...
		double price = fast_atof_p(price_start);
		long quantity = fast_atol_p(c2 + 1);
		if (quantity > 0 && price > 10.0) {
