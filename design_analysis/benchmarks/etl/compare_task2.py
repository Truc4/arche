#!/usr/bin/env python3
"""Compare Arche task2 (filter invalid) with Pandas for correctness and performance."""
...
def run_arche(iterations=10):
    """Run Arche task2 and measure time over multiple iterations."""
    # Create output directory
    os.makedirs('build/benchmarks/etl/task2', exist_ok=True)
    ...
    for i in range(iterations):
        start = time.perf_counter()
        result = subprocess.run(['build/benchmarks/etl/task2/task2_arche_bin'], capture_output=True, text=True)
    ...
    arche_data = []
    with open('build/benchmarks/etl/task2/arche_output.csv') as f:
        reader = csv.reader(f)
        for row in reader:
            arche_data.append((float(row[0]), int(row[1]), int(row[2])))

    return arche_data, arche_compile_time, times

def run_pandas(iterations=10):
    """Run Pandas task2 and measure time over multiple iterations (including write)."""
    times = []
    for i in range(iterations):
        start = time.perf_counter()
        df = pd.read_csv('design_analysis/benchmarks/etl/data/data.csv')
        df['valid'] = (df['quantity'] > 0).astype(int)
        df_output = df[['price', 'quantity', 'valid']].head(1000)
        df_output.to_csv('build/benchmarks/etl/task2/pandas_output.csv', index=False, header=False)
        elapsed = time.perf_counter() - start
        times.append(elapsed)

    pandas_data = [(row['price'], row['quantity'], row['valid'])
...
def compare_data(arche_data, pandas_data):
    ...
    for i, (arche_row, pandas_row) in enumerate(zip(arche_data, pandas_data)):
        arche_price, arche_qty, arche_valid = arche_row
        pandas_price, pandas_qty, pandas_valid = pandas_row

        if (abs(arche_price - pandas_price) < tolerance and
            arche_qty == pandas_qty and
            arche_valid == pandas_valid):
            matches += 1
        else:
            if len(mismatches) < 5:
...
def main():
    ...
    print(f"Running Arche task2 ({iterations} iterations)...")
    arche_data, arche_compile, arche_times = run_arche(iterations)

    print(f"Running Pandas task2 ({iterations} iterations)...")
