#!/usr/bin/env python3
"""Task 4 (Polars, transform-only): sum(price * quantity) scalar, repeated N times."""
...
def main(csv_path):
    ...
    for _ in range(N_ITERS):
        total += df.select((pl.col("price") * pl.col("quantity")).sum()).item()
    elapsed = time.perf_counter() - start

    os.makedirs(OUT_DIR, exist_ok=True)
    pl.DataFrame({"total": [total]}).write_csv(f"{OUT_DIR}/task_4.csv")
