#!/usr/bin/env python3
"""Task 4 (DataFusion, transform-only): sum(price * quantity), repeated N times. In-memory table."""
...
def main(csv_path):
    ...
    for _ in range(N_ITERS):
        batches = ctx.sql("SELECT SUM(price * quantity) FROM t").collect()
        total += batches[0].column(0)[0].as_py()
    elapsed = time.perf_counter() - start

    os.makedirs(OUT_DIR, exist_ok=True)
    with open(f"{OUT_DIR}/task_4.csv", "w") as f:
        f.write(f"total\n{total}\n")
