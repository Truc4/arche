/* Task 2 (C, transform-only, branchless): count(quantity > 0) without `if`.
 * `count += (quantity[i] > 0)` — bool→int gives 0 or 1, summed with no branch.
 * Auto-vectorizable; on uniformly-true predicates the branched version is
 * effectively branch-predicted-free, so this is mostly an SIMD test.
 */
...
#define OUT_PATH "design_analysis/benchmarks/transform/out/c/task_2_branchless.csv"
