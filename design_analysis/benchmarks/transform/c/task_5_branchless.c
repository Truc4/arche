/* Task 5 (C, transform-only, branchless): filter q>0 AND p>10, sum(p*q), no `if`.
 * Mask becomes arithmetic: total += (mask ? 1 : 0) * price * quantity.
 * Auto-vectorizable; can use AVX2 conditional-blend or just plain SIMD multiply.
 */
...
#define OUT_PATH "design_analysis/benchmarks/transform/out/c/task_5_branchless.csv"
