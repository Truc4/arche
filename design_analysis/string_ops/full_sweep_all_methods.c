...
void m2_shrink(M2 m) {
	char *newbuf = malloc(m.cap);
	int npos = 0;
	for (int i = 0; i < m.cnt; i++) {
		int newlen = m.len[i] / 2;
		if (newlen < 0)
			newlen = 0;
		memcpy(newbuf + npos, m.buf + (npos), newlen);
		npos += newlen + 1;
		m.len[i] = newlen;
	}
	memcpy(m.buf, newbuf, npos);
	m.sz = npos;
	free(newbuf);
}
void m2_free(M2 m) {
...
void m3_shrink(M3 m) {
	...
	for (int i = 0; i < m.cnt; i++) {
		...
		if (newlen < 0)
			newlen = 0;
		memcpy(newbuf + npos, m.buf + m.off[i], newlen);
		npos += newlen + 1;
		m.off[i] = npos - newlen - 1;
		m.len[i] = newlen;
	}
	memcpy(m.buf, newbuf, npos);
	m.sz = npos;
...
int main() {
	FILE *csv = fopen("results/full_sweep_all_methods.csv", "w");
	fprintf(csv, "lifecycle,fixed,grow,shrink,m1_ns,m2_ns,m3_ns,m4_ns,winner\n");

	int seed = 42;
	printf("Full sweep ALL methods WITH SHRINK...\n");
	...
	for (int lc = 0; lc <= 100; lc += 20) {
		for (int fx = 0; fx <= 100 - lc; fx += 20) {
			...
			printf("lc:%3d fx:%3d gr:%2d sh:%2d | M1:%8lld M2:%8lld M3:%8lld M4:%8lld | W:%d\n", lc, fx, gr, sh,
			       t1 / ITERS, t2 / ITERS, t3 / ITERS, t4 / ITERS, w);
			fprintf(csv, "%d,%d,%d,%d,%lld,%lld,%lld,%lld,%d\n", lc, fx, gr, sh, t1 / ITERS, t2 / ITERS, t3 / ITERS,
			        t4 / ITERS, w);
		}
	}

	fclose(csv);
	printf("Saved to results/full_sweep_all_methods.csv\n");
	return 0;
}
