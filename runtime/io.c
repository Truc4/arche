...
static int pr_nthreads(long long n) {
	...
	if (ncpu < 1) ncpu = 1;
	if (ncpu > PR_MAX_THREADS) ncpu = PR_MAX_THREADS;
...
#define PR_GEN(NAME, T, ID_MIN, ID_MAX)                                                                  \
	typedef struct { const T *col; long long lo, hi; int op; T out; } NAME##_arg;                       \
	static T NAME##_fold(const T *col, long long lo, long long hi, int op) {                            \
		...
		for (long long i = lo; i < hi; i++) {                                                           \
			T v = col[i];                                                                               \
			if (op == PR_ADD) acc = acc + v;                                                            \
			else if (op == PR_MUL) acc = acc * v;                                                       \
			else if (op == PR_MIN) { if (v < acc) acc = v; }                                            \
			else if (v > acc) acc = v;                                                                  \
		}                                                                                               \
		return acc;                                                                                     \
	}                                                                                                   \
	static void *NAME##_worker(void *p) { NAME##_arg *a = (NAME##_arg *)p;                              \
		a->out = NAME##_fold(a->col, a->lo, a->hi, a->op); return 0; }                                  \
	T arche_par_reduce_##NAME(const T *col, long long n, int op) {                                      \
		T id = (op == PR_ADD) ? (T)0 : (op == PR_MUL) ? (T)1 : (op == PR_MIN) ? (T)(ID_MIN) : (T)(ID_MAX); \
		if (n <= 0) return id;                                                                          \
		int k = pr_nthreads(n);                                                                         \
		if (k <= 1) return NAME##_fold(col, 0, n, op);                                                  \
		pthread_t th[PR_MAX_THREADS];                                                                   \
		NAME##_arg args[PR_MAX_THREADS];                                                                \
		long long chunk = n / k;                                                                        \
		int spawned = 0;                                                                                \
		for (int t = 0; t < k; t++) {                                                                   \
			args[t].col = col; args[t].op = op;                                                         \
			args[t].lo = (long long)t * chunk;                                                          \
			args[t].hi = (t == k - 1) ? n : (long long)(t + 1) * chunk;                                 \
			if (pthread_create(&th[t], 0, NAME##_worker, &args[t]) == 0) spawned++;                     \
			else args[t].out = NAME##_fold(col, args[t].lo, args[t].hi, op);                            \
		}                                                                                               \
		for (int t = 0; t < spawned; t++) pthread_join(th[t], 0);                                       \
		T acc = args[0].out;                                                                            \
		for (int t = 1; t < k; t++) { T v = args[t].out;                                                \
			if (op == PR_ADD) acc = acc + v;                                                            \
			else if (op == PR_MUL) acc = acc * v;                                                       \
			else if (op == PR_MIN) { if (v < acc) acc = v; }                                            \
			else if (v > acc) acc = v;                                                                  \
